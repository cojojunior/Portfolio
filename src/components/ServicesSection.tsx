...
const ServicesSection = () => {
  ...
  return (
    <section className="py-1 md:py-1 lg:py-1 bg-white">
      <div className="container mx-auto px-4">
        {/* Section Header */}
              ...
              {service.features && service.features.length > 0 && (
                <div className="flex flex-wrap gap-1 md:gap-1.5">
                  {service.features.slice(0, 2).map((feature, idx) => (
                    <span
                      key={idx}
                      className="px-1.5 md:px-2.5 py-0.5 bg-golden/10 text-dark-navy rounded text-[8px] md:text-xs">
                      {feature}
                    </span>
                  ))}
                  {service.features.length > 2 && (
                    <span className="px-1.5 md:px-2.5 py-0.5 bg-golden/10 text-dark-navy rounded text-[8px] md:text-xs">
                      +{service.features.length - 2}
                    </span>
                  )}
                </div>
